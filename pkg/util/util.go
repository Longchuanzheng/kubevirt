...
// Check if a VMI spec requests Localtime clock and set to disable migrate
func IsLocalTimeDisableMigratable(vmi *v1.VirtualMachineInstance) bool {
	if vmi.Spec.Domain.Clock != nil && vmi.Spec.Domain.Clock.LocalTime != nil {
		return *(vmi.Spec.Domain.Clock.LocalTime.DisableMigrate)
	}
	return false
}
